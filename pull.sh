...
function pull () {
	...
	rsync -e "ssh -p ${SSH_PORT}" -rlptvz --delete --exclude .git/ --exclude .gitignore --exclude .sass-cache/ --exclude bin/ --exclude 'tmp/*' --exclude wp-config.php --exclude node_modules/ --exclude .DS_Store --exclude '*.sql' ${exclude} --exclude demo/ ${SSH_USER}@${SSH_HOST}:${WORDPRESS_PATH}${path} ${LOCAL_WORDPRESS_PATH}${path}
}
